...
class ExecuterGenerator
  ...
  def translate_stmt(stmt, indent)
    ...
    when :setpc then 
        "#{indent}hart.set_next_pc(#{get_var_name(stmt.oprnds[0])});
        if(jit_compiler->is_compiled(#{get_var_name(stmt.oprnds[0])})) {
            jit_compiler->run_compiled(#{get_var_name(stmt.oprnds[0])});
        } else {
            jit_compiler->increase_access_rate_and_compile_if_ready(#{get_var_name(stmt.oprnds[0])}, hart);
        }"
    when :if_expr
