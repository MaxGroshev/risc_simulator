...
class DecoderGenerator
  ...
  def generate_decoding_methods
    ...
    @data['instructions'].each do |name, instr|
      methods << "static void decode_#{name}(uint32_t instruction, DecodedInstruction& result);"
    end
    methods.join("\n    ")
  end
  ...
  def generate_instruction_decoders
    ...
    @data['instructions'].each do |name, instr|
      decoder = <<~CPP
        void RV32IDecoder::decode_#{name}(uint32_t instruction, DecodedInstruction& result) {
  ...
  def generate_opcode_dispatch
    ...
    opcode_groups.each do |opcode, instructions|
      case_stmt = "case #{opcode}:"
      if instructions.size == 1
        name = instructions.first[0]
        case_stmt += "\n decode_#{name}(instruction, result); break;"
      else
