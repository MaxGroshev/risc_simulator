...
Hart::Hart(Machine& machine, uint32_t cache_len)
    : machine_(machine), pc_(0), next_pc_(0), halt_(false), block_cache_(4096), cache_len_(cache_len) {
    regs_.fill(0);
}
...
uint64_t Hart::step() {
    ...
    while (collected < cache_len_) {
        ...
        uint32_t executed_next = next_pc_;
