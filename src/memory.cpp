...
uint32_t Memory::read(uint32_t addr, int size, bool sign_extend) const {
    if (addr + size > mem_.size()) throw std::out_of_range("Memory read out of bounds");
    uint32_t value = 0;
...
void Memory::load_data(uint32_t addr, const uint8_t* data, size_t size) {
    if (addr + size > mem_.size()) throw std::out_of_range("Memory load out of bounds");
    std::memcpy(&mem_[addr], data, size);
}

void Memory::zero_init(uint32_t addr, size_t size) {
    if (addr + size > mem_.size()) throw std::out_of_range("Memory zero-init out of bounds");
